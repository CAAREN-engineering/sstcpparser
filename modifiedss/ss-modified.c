...
/* added to force output of booleans and TCP congestion control algo, all as key:value pairs*/

        if (s->cong_alg[0])
                out("cong_alg:%s", s->cong_alg);

	out(" ts:");
	out(s->has_ts_opt ? "True" : "False");

	out(" usec_ts:");
	out(s->has_usec_ts_opt ? "True" : "False");

	out(" sack:");
	out(s->has_sack_opt ? "True" : "False");

	out(" ecn:");
	out(s->has_ecn_opt ? "True" : "False");
...
/* modified to force key:value output of this boolean */
	out(" app_limited:");
        out(s->app_limited ? "True" : "False");
/* end modification */
